/*
  ...
  config = {
    ...
    pirateDeck: null          // opcional: [{name, count}] — mazo real sin repetición para el botón "Carta Pirata"
  }
*/
function initScorer(config){
  ...
  function save(){
    localStorage.setItem(KEY, JSON.stringify({players, locked, deck, currentRound, totalRounds}));
  }
  ...
  function startGame(){
    ...
    for(let i=1;i<=config.maxPlayers;i++){
      ...
      if(val) players.push({name: val, total: 0, history: [], phase: 1, phaseDone: false, milestoneAnnounced: false, phaseAnnounced: false, completeAnnounced: false, pirateCard: null});
    }
  ...
  function resetScores(){
    showModal({
      ...
      onConfirm: () => {
        clearTimeout(phaseAnnounceTimer);
        clearTimeout(completeAnnounceTimer);
        players.forEach(p => { p.total = 0; p.history = []; p.phase = 1; p.phaseDone = false; p.milestoneAnnounced = false; p.phaseAnnounced = false; p.completeAnnounced = false; p.pirateCard = null; });
        if(config.pirateDeck) deck = shuffle(buildFullDeck());
  ...
  function render(){
    ...
    players.forEach((p, idx) => {
      ...
      if(config.pirateDeck && !locked){
        pirateHtml = `
          <button class="btn small pirate-btn" onclick="scorerDrawPirateCard(${idx})">🏴 Carta Pirata</button>
          ${p.pirateCard ? `<div class="pirate-card">🃏 ${p.pirateCard}</div>` : ''}`;
      }
      div.innerHTML = `
